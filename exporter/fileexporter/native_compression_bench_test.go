...
func BenchmarkZstdExportLevels(b *testing.B) {
	td := generateLargeTraces()
	for _, level := range []int{0, 3, 7, 12, 22} {
...
func benchExportTraces(b *testing.B, format, compression string, level int, td ptrace.Traces) {
	b.Helper()
...
func benchExportLogs(b *testing.B, format, compression string, level int, ld plog.Logs) {
	b.Helper()
